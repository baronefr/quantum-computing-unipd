...
contains
    ...
    !   MATH |  EIGENVALUES
    ! ------------------------------------------------------------------
    !        Input   |   cmx         complex matrix (square)
    ...
    !  Requirements  |   cmx must be initialized
    ! 
    !        Output  |   eigv        the double array of eigenvalues
    ! 
    ...
    ! ------------------------------------------------------------------
    function Eigenvalues(cmx, uselower) result(eigv)
        implicit none
        ! args
        type(complex16_matrix), intent(in) :: cmx
        logical, optional, intent(in) :: uselower
        ...
        if(present(uselower)) then
            ...
            uplo_ = 'L' ! default: use lower triangular
        end if
        ! this procedure is shamelessly inspired from LAPACK documentation:
        ! https://numericalalgorithmsgroup.github.io/LAPACK_Examples/examples/doc/zheev_example.html

        if( cmx%size(1) .eq. cmx%size(2) ) then
            n = cmx%size(1);   lda = n;
            allocate( rwork(3*n-2), eigv(n))
            
            ! use routine workspace query to get optimal workspace
            lwork = -1
            call zheev('N', uplo_, n, cmx%val, lda, eigv, dummy, lwork, rwork, info)
            ...
            call zheev('N', uplo_, n, cmx%val, lda, eigv, work, lwork, rwork, info)

            if(info==0) Then
                call checkpoint(level=DL_INFOP, msg = 'eigenvalues ok')
            else
                call checkpoint(level=DL_FATAL, msg = 'eigenvalues not computed')
                stop '(fatal error)'
            end if
    ...
    subroutine CMatDumpTXT(cmx, filename)
        ...
        if(file_isopened) then
            ...
            ! write to file
            do ii = 1, cmx%size(1)
                write(fileunit, *) cmx%val(ii, :)   ! naive
                !write(fileunit, '(*(F14.7))')(cmx%val(ii, jj), jj=1,cmx%size(2) )
            end do
        else
