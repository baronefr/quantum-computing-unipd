...
contains
    ...
    ! write an array to txt file line by line
    subroutine write_array_txt(filename, array, first_index, last_index, append)
        implicit none
        ...
        if (trim(filename) == '') then
            ...
            if(append) then
                call checkpoint(level=DL_PEDANTIC, msg = "opening file "//trim(filename)//" -> append")
                open(fileunit, file=filename, action='write', position='append')
            else
                call checkpoint(level=DL_PEDANTIC, msg = "opening file "//trim(filename)//" -> write")
